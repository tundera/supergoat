...
export const isJsonApiResponse = <T = unknown>(val: unknown): val is JsonApiResponse<T> => {
  return isPlainObject(val) && 'success' in val && typeof val.success === 'boolean'
}
...
export const isJsonApiErrorResponse = (val: unknown): val is JsonApiErrorResponse => {
  return (
